...
-define( ONE_SECOND        , 1000000 ). % 1 second in microseconds
-define( DEFAULT_FRAME_TIME, 60 ). % default number of frames to run per second
...
frame_limit( StartTime, FrameTime ) ->
    ...
    SleepTime = (FrameTime - TimeDiff), % typically it actually runs at half speed, so half the sleep time to counter this
    if
        (SleepTime > 1000) ->
